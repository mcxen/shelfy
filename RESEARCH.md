...
| `extract` / `compress` | Shelfy 扩展 | ZIP 内置解压/压缩；7z/RAR 调系统 7-Zip；支持密码列表、7z.01/7z.001 分卷完整性预检；仅成功后删除源包/整组源卷 | ✗ | ✓ | compress ✓ |
...
- **archive 动作**：作为 Shelfy 扩展实现 `extract`/`compress`。ZIP 由 `zip` crate 内置处理，解压前全量读取条目并校验 CRC；7z/RAR 调用系统 `7z` / `7zz` / `7za`，先执行 `7z t`，测试通过后才执行 `7z x`。Windows 除 PATH 外也探测 `ProgramW6432` / `ProgramFiles` / `ProgramFiles(x86)` 下的标准 `7-Zip\7z.exe`。`archivefile` 过滤器、`extract format: auto` 与递归解压均读取文件头魔数识别 ZIP/7z/RAR，不依赖普通扩展名；分卷 7z 仍通过卷号定位首卷后读取首卷文件头。`compress format: auto` 按目标文件名选择格式，无法识别时回退 ZIP。simulate/执行前确认预检也会完成 ZIP CRC 或 `7z t` 校验并试配候选密码，但不创建目标目录、不解压、不删除源包。`recursive=true` 可继续解压本次新产生的内部压缩包，使用路径去重、不跟随符号链接、1–10 层深度和 1000 包总量上限避免循环或扩张失控。收到任意 `.7z.02/.002` 等后续卷时会按相同位数自动定位 `.01/.001` 首卷，再检查卷号是否从 1 连续；首卷缺失则保留全部文件并报错。`delete_original` 只在完整性测试和解压都成功后删除，分卷 7z 会删除同一组的全部卷。密码参数按产品要求在 Orden 日志中明文显示。系统未安装 7-Zip 时动作报错且保留源文件。
