...
- **archive 动作**：作为 Shelfy 扩展实现 `extract`/`compress`。ZIP 由 `zip` crate 内置处理，解压前全量读取条目并校验 CRC；7z/RAR 调用系统 `7z` / `7zz` / `7za`，先执行 `7z t`，测试通过后才执行 `7z x`。自动识别 `.7z.01` / `.7z.001` 首卷，拒绝从非首卷启动并检查本地卷号是否连续；`delete_original` 只在完整性测试和解压都成功后删除，分卷 7z 会删除同一组的全部卷。密码参数在 Orden 日志中脱敏。系统未安装 7-Zip 时动作报错且保留源文件。
