...
- **archive 动作**：作为 Shelfy 扩展实现 `extract`/`compress`。ZIP 由 `zip` crate 内置处理，解压前全量读取条目并校验 CRC；7z/RAR 调用系统 `7z` / `7zz` / `7za`，先执行 `7z t`，测试通过后才执行 `7z x`。Windows 除 PATH 外也探测 `ProgramW6432` / `ProgramFiles` / `ProgramFiles(x86)` 下的标准 `7-Zip\7z.exe`。simulate/执行前确认预检也会完成 ZIP CRC 或 `7z t` 校验并试配候选密码，但不创建目标目录、不解压、不删除源包。`recursive=true` 可继续解压本次新产生的内部压缩包，使用路径去重、不跟随符号链接、1–10 层深度和 1000 包总量上限避免循环或扩张失控。收到任意 `.7z.02/.002` 等后续卷时会按相同位数自动定位 `.01/.001` 首卷，再检查卷号是否从 1 连续；首卷缺失则保留全部文件并报错。`delete_original` 只在完整性测试和解压都成功后删除，分卷 7z 会删除同一组的全部卷。密码参数在 Orden 日志中脱敏。系统未安装 7-Zip 时动作报错且保留源文件。
